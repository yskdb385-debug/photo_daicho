...
async function analyzeWhiteboard(file) {
  ...
  if (!bestPixels || bestSize < 20) {
    return { score: 0, brightFraction, edgeDensity: 0, ok: true };
  }
  // bounding box
  ...
  return { score, brightFraction, edgeDensity, ok: true };
}
...
async function runProcessing() {
  ...
  for (const g of wbGroups) {
    ...
    if (wb) {
      g.ocrRaw = await ocrPhoto(wb.thumbBlob || wb.blob);
...
function wireReviewEvents() {
  ...
  document.querySelectorAll('[data-claude]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      try {
        const fields = await claudeEnhanceFields(wb.thumbBlob || wb.blob);
